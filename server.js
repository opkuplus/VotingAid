  ...
  Candidate.find(
    ...
    function (err, results) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        for (
          var j = 0;
          j < Object.keys(results[i].filledForm).length / 2;
          j++
        ) {
          if (userAnswer[j] === results[i].filledForm["question" + j]) {
          ...
          if (
            (userAnswer[j] == 2 &&
              results[i].filledForm["question" + j] == -2) ||
            (userAnswer[j] == -2 && results[i].filledForm["question" + j] == 2)
          ) {
            danger += 1; //Check for opposite answer
          }
        }
        let userSum = userAnswer.reduce((result, number) => result + number);
        let candSum = candidateArray.reduce(
          (result, number) => result + number
        );

        if (
          (userSum >= candSum * 0.5 || danger <= 0.25 * userAnswer.length) &&
          similarity >= 0.3 * userAnswer.length
        ) {
...
app.post("/filteredCandidates", (req, res) => {
  //Shows filtered andidates
...
app.post("/questions", (req, res) => {
  //Form question and parse call
  const area = req.body.data;
  Question.find(
    { area: { $in: [area, "Undefined"] } },
    function (err, results) {
      res.send(results);
    }
  );
...
app.post("/filteredQuestions", (req, res) => {
...
app.post("/submitQhuahoo", function (req, res) {
  //EDIT ONE EXISTING submitQhuahoo
...
app.post("/deleteQhuahoo", function (req, res) {
  //DELETE ONE EXISTING Qhuahoo
  var id = req.body.id;
  ...
  bcrypt.hash(pass, saltRounds, function (err, hash) {
    ...
    Candidate.countDocuments({ email: req.body.email }, function (err, count) {
      if (count !== 0) {
        ...
        res.send(
          "You cannot register an account for email address that does not exist as a candidate!"
        );
      }
    });
  });
      ...
      bcrypt.compare(pass, user.password).then(function (result) {
        if (result) {
          ...
          const token = jwt.sign(tokenUser, process.env.TOKEN_SECRET, {
            expiresIn: 60 * 60,
          }); // CHANGE TO 60 FOR TESTING PURPOSES
...
app.post("/send", function (req, res) {
  ...
  try {
    ...
    return res.status(401).send("Error!, TOKEN HAS EXPIRED OR IS INVALID");
  }
  ...
  for (var i = 0; i < req.body.ans.length; i++) {
    ...
    Candidate.findOneAndUpdate(
      { email: email },
      {
        $set: { [nestedOpt]: req.body.ans[i], [nestedDesc]: req.body.desc[i] },
      },
      { useFindAndModify: false },
      function (err, doc) {
        console.log(doc);
...
app.get("/initPictures", function (req, res) {
  Candidate.find({}, function (err, allCandidate) {
    allCandidate.forEach((oneCandidate) => {
      ...
      Question.find({}, function (err, cookie) {
